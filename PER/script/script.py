...
def extract_mitre_data(mitre_json):
    ...
    for cve in mitre_json:
        ...
        extracted_data[cve_id] = {
            ...
            "Base_Score": None if cve.get("Base_Score") in ["No score available", None] else cve.get("Base_Score")
        }
...
def merge_cve_data(mitre_data, nvd_data):
    ...
    for cve_id, mitre_entry in mitre_data.items():
        if cve_id in merged_data:
            # 🔹 Compléter la description si elle est absente
            if merged_data[cve_id]["Description"] in [None, "No description available"]:
            ...
            if merged_data[cve_id]["Base_Score"] in [None, "No score available", "null"]:
                merged_data[cve_id]["Base_Score"] = mitre_entry["Base_Score"]
            elif mitre_entry["Base_Score"] not in [None, "No score available", "null"]:
                # Comparer les scores et prendre le max
