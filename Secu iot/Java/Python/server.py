    ...
    try:
        ...
        if request.method == 'POST':
            ...
            try:
                ...
                # Concaténer toutes les transactions en une seule chaîne
                decoded_transactions = [base64.b64decode(tx) for tx in filtered_transactions]
                concatenated_transactions = b''.join(decoded_transactions)


                # Envoyer les transactions concaténées à la carte
                decrypted_logs_str = decrypt_logs(connection, concatenated_transactions)

                # Ajouter un saut de ligne avant chaque date au format YYYY-MM-DD HH:MM:SS
                formatted_transactions = re.sub(r"(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})", r"\n\1", decrypted_logs_str)
